 ...
 */
function firestoreToMailDoc(firestoreData) {
  ...
  const convertArray = (field) => {
    if (field?.arrayValue?.values) {
      return field.arrayValue.values.map((v) => v.stringValue || v);
    }
    return field || [];
  };
  ...
  return {
    ...
    status: firestoreData.status?.stringValue || 'queued',
 ...
 */
async function acquireLease(docId, leaseOwner) {
  try {
 ...
 */
async function releaseLease(docId, status, attempts, error) {
  try {
    const docRef = db.collection('mailQueue').doc(docId);
    const updateData = {
      status,
 ...
 */
async function sendEmail(gmail, mailDoc) {
  try {
 ...
 */
async function processMailDocument(docId, mailDoc) {
 ...
 */
exports.onMailQueued = async function(cloudEvent) {
  try {
 ...
 */
exports.onMailRequeued = async function(cloudEvent) {
  try {
