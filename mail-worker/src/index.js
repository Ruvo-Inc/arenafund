...
async function tryAcquireLease(ref) {
    ...
    const ok = await db.runTransaction(async (tx) => {
        ...
        if (d.status !== "queued")
            return false;
        const notBefore = d.notBefore ?? null;
        if (notBefore && notBefore.toMillis() > now.toMillis())
            return false;
        const leaseExpiresAt = d.leaseExpiresAt ?? null;
...
function buildRfc822(opts) {
    ...
    if (opts.replyTo && opts.replyTo.trim())
        headers.push(`Reply-To: ${opts.replyTo.trim()}`);
...
async function gmailClient() {
    ...
    const jwtClient = new google.auth.JWT(GMAIL_CLIENT_EMAIL, undefined, GMAIL_PRIVATE_KEY.replace(/\\n/g, '\n'), // Handle escaped newlines
    ['https://www.googleapis.com/auth/gmail.send', 'https://www.googleapis.com/auth/gmail.compose'], GMAIL_SUBJECT_USER);
    return google.gmail({ version: "v1", auth: jwtClient });
}
async function sendWithGmail(job) {
...
export async function handleMailQueueEvent(snap) {
    const ref = snap.ref;
